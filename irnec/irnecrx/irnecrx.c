...
static int read_timeout = 1000;    // ms
...
static rmt_receive_config_t receive_config = {
    ...
    // the shortest duration for NEC signal is 560us,
    // 1250ns < 560us, valid signal won't be treated as noise
    .signal_range_max_ns = 12000000,
    // the longest duration for NEC signal is 9000us,
    // 12000000ns > 9000us, the receive won't stop early
};
static QueueHandle_t receive_queue = NULL;
// save the received RMT symbols
// 64 symbols should be sufficient for a standard NEC frame
...
void xs_irnecrx(xsMachine *the)
{
	...
	rx_channel_cfg.gpio_num = xsmcToInteger(xsVar(0));

    // set read_timeout
 	if (xsmcHas(xsArg(0), xsID_timeout)) {
		xsmcGet(xsVar(0), xsArg(0), xsID_timeout);
		read_timeout = xsmcToInteger(xsVar(0));
    }
